...
:::{.problem title="?"}
a.
Show that for each $n \geq 2$, the group $\SO_n$ has a unique connected 2 -fold covering space. Call this space $\Spin(n)$

b.
Use the group structure of $\SO_n$ to induce a group structure on $\Spin(n)$.

c.
What familiar space is $\Spin(3) ?$ Also, show that as topological groups, $\Spin(3) \cong S U(2)$.
...
a.
Suppose that $F: \cat{C} \to \cat{D}$ is a functor. Show that there is an induced continuous map $\mathcal{N F}$ : $ \nerve{\cat C} \to \nerve{\cat D}$ of nerves.
