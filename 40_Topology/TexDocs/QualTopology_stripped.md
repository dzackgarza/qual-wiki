...
-   Loops can be homotopic (i.e. *freely* homotopic) without being homotopic rel a base point, so not equal in \( \pi_1(X; x_0) \).
    ...
    ![](figures/2020-02-04-20:00.png "fig:")
:::
...
Let \( X \) be the topological space obtained as the quotient of the sphere \( S^2 = \left\{{\mathbf{x} \in {\mathbb{R}}^3 {~\mathrel{\Big\vert}~}{\left\lVert {\mathbf{x}} \right\rVert} = 1}\right\} \) under the equivalence relation \( \mathbf{x} \sim -\mathbf{x} \) for \( \mathbf{x} \) in the equatorial circle, i.e. for \( \mathbf{x} = (x_1, x_2, 0) \).
...
a.  Consider the quotient space
    ...
    ,\]
    and let \( A \) be any \( 2 \times 2 \) matrix whose entries are integers such that \( \operatorname{det}A = 1 \).
